...
import { DEMO_USERS, login, useAuth } from "@/lib/auth";
...
export const Route = createFileRoute("/login")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Sign in to the FlowPilot AI demo workspace." },
    ],
  }),
  component: LoginPage,
});
      ...
      <div className="hidden lg:flex flex-col justify-between p-12">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
          <h1 className="text-5xl font-semibold tracking-tight leading-tight">
            ...
            <span className="brand-text">Amplify Teams.</span>
          </h1>
          <p className="mt-6 text-muted-foreground max-w-md">
            One AI assistant across CRM, HR, projects, and finance. Stream natural-language commands,
            score leads, screen resumes, flag project risk, and chase invoices — all in one workspace.
          </p>
          <div className="mt-8 flex flex-wrap gap-2">
            {["CRM", "Recruiting", "Projects", "Finance", "Audit"].map((t) => (
              <Badge key={t} variant="secondary">{t}</Badge>
            ))}
          </div>
        </motion.div>
        <p className="text-xs text-muted-foreground">© 2026 FlowPilot AI · Contest demo</p>
      </div>
          ...
          <Card className="glass shadow-elegant">
            <CardContent className="p-8">
              <div className="lg:hidden flex items-center gap-2 mb-6">
                ...
                <span className="font-semibold">FlowPilot AI</span>
              </div>
              <h2 className="text-2xl font-semibold tracking-tight">Sign in</h2>
              <p className="text-sm text-muted-foreground mt-1">Use a demo account to explore the workspace.</p>
              ...
              <div className="mt-6">
                <p className="text-xs uppercase tracking-wide text-muted-foreground mb-2">Demo accounts (click to fill)</p>
                ...
                <p className="text-[10px] text-muted-foreground mt-3">Password for all demo accounts: <code>password123</code></p>
              </div>
            </CardContent>
          </Card>
        </motion.div>
